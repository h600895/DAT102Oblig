...
public class BS_Tre<T extends Comparable<? super T>> implements SoektreInterface<T> {
	...
	public int finnLengden() {
		//Kaller på en rekursiv metode.
	}
	public int nesteBlad(int teller) {
		if (rot.getVenstre() == null && rot.getHogre() ==  null) {
			return rot;
		}
		BinaerTreNode<T> venstreRot = nesteBlad(rot.getVenstre());
		BinaerTreNode<T> hoyreRot = nesteBlad(rot.getHogre());
