...
public class TabellMengde<T> implements MengdeADT<T> {
	...
	public MengdeADT<T> snitt(MengdeADT<T> m2) {
		...
		T element = null;
		Iterator<T> mengde2 = m2.oppramser(); 
	...
	@Override
	public MengdeADT<T> differens(MengdeADT<T> m2) { //Antar: this\m2
		//TODO
		MengdeADT<T> differensM = new TabellMengde<T>();
		T element;
		MengdeADT<T> snitt = snitt(m2);
		for (int i = 0; i < antall; i++) {
			if (!snitt.inneholder(tab[i])) {
	...
	public boolean undermengde(MengdeADT<T> m2) {
		...
		MengdeADT snitt = snitt(m2);
		Iterator<T> mengde2 = m2.oppramser();
		while (mengde2.hasNext()) {
			if (!snitt.inneholder(mengde2.next())) {
				return false;
			}
		}
